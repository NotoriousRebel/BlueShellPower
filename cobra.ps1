...
function dump_tasks{
	$cur = $env:USERNAME
	Write-Verbose -Message "Putting scheduledtasks into tasks.txt"
	tasklist | Out-File "C:\Users\$($cur)\Desktop\Storage\tasks.txt"
	Write-Verbose -Message "Putting scheduledtask information into tasksinfo.txt"
	...
	cmd.exe /c "sc query state=all" | Out-File "C:\Users\$($cur)\Storage\sc_query.txt"
	Write-Verbose -Message "Flushing scheduled tasks"
	try{
...
function app_lock{
	...
	Catch{
			Write-Verbose -Message "Sleeping 30 seconds for user to start AppIDSvc service manually in services.msc or net start AppIDSvc"
			Sleep -Seconds 30
	...
	Catch{
		Write-Verbose -Message "Could not change startup type for AppIDSvc service do it manually."
	}
	$local = @" 
...
function install_packages{	
	...
	# Remove prompt that chocolatey has when installing something
...
function harden{
	# Aspects of this function are inspired by https://gist.github.com/jaredhaight/e88b4323adce06395dace501841d3075
	...
	Try{
		...
		#comment these out if you want to disable smbv2 and smbv3 
	}
	Catch{
		$string_err = $_ | Out-String
		Write-Verbose -Message $string_err -verbose
	}
	Write-Verbose -Message "Disabling RDP!!!" 
	Try{
...
function enable_logging{
	...
	Catch{
		Write-Verbose -Message "Could not enable logging, will have to do manually" 
	...
	# get program to block an ip and store it on desktop
}
...
function main{
	...
	Write-Verbose -Message "**********\n\n\n"
	Write-Verbose -Message "Creating directory on $($env:USERNAME)'s Desktop"
	cd ~\Desktop
	New-Item -Path "~\Desktop" -Name "Storage" -ItemType "directory"
	change_users
	harden
	enable_logging_and_guard
	install_chocolate
	install_packages
	dump_tasks
	read_history
